...
class TestOrderBook(unittest.TestCase):
    ...
    def test_add_sorted(self):
        ...
        self.assertTrue(is_bids_sorted(ob1))
        ...
        self.assertTrue(is_bids_sorted(ob1))
        self.assertEqual(len(ob1.asks), 3)


    def test_bids_sorted(self):
        ob2 = OrderBook()
        ob2.add_order(Order(100,'bid'))
        self.assertTrue(is_bids_sorted(ob2))
        self.assertEqual(len(ob2.bids), 1)
        ob2.add_order(Order(120,'bid'))
        print(ob2)
        self.assertTrue(is_bids_sorted(ob2))
        self.assertEqual(len(ob2.bids), 2)
        ob2.add_order(Order(40, 'bid'))
        self.assertTrue(is_bids_sorted(ob2))
        self.assertEqual(len(ob2.bids), 3)

    def test_bid_asks_sorted(self):
        ob3 = OrderBook()
        ob3.add_order(Order(100,'bid'))
        ob3.add_order(Order(100,'ask'))
        self.assertTrue(is_bids_sorted(ob3))
